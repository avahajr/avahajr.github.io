...
test.describe("validate PDF contents", () => {
  let pdfContents: any;
  ...
  test("keywords should be correct", async () => {
    expect(
      pdfContents.Meta.Keywords.split(", "),
      "PDF keyword was incorrect",
    ).toContain("software engineer");
  });

  test("author should be correct", async () => {
    expect(pdfContents.Meta.Author, "PDF author was incorrect").toEqual(
      "Ava Hajratwala",
    );
  });
  ...
  test("contains the correct heading text", async () => {
    ...
    expect(decodeURI(heading), "The heading text was incorrect").toEqual(
      "AvaHajratwala",
    );
  });
});
