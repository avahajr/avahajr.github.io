...
test.describe("Navbar scrollspy functionality", () => {
  ...
  for (const navItem of [
    ...
    test(`Clicking on ${navItem.label} scrolls to the correct section`, async ({
      page,
    }) => {
      ...
      test.skip(
        !isVisible,
        `${navItem.label} link is not visible, skipping test.`,
      );
      ...
      await expect
        .poll(async () => await page.title(), { timeout: 5000 })
        .toContain(navItem.label);
      ...
      if (boundingBox) {
        ...
        expect(boundingBox.y).toBeGreaterThanOrEqual(-0.3);
